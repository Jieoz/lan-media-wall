...
> **v1.19.3 候选：播放控制按意图分组。** “播放”保留上一项/暂停/继续/下一项/停止；
> “播放模式”用单一分段选择器呈现图片/视频与音乐终端；“音乐列表”只负责打开编辑器；
> “电源”按真实状态显示待机或退出待机。音乐弹窗只上传、排序、保存列表，不再复制
> 主控制区的模式按钮。单一版本源为 `1.19.3+1193`。
> `runtime_mode_result` 只有与当前设备最新待处理 request_id 严格关联时才立即并入状态墙；
> 迟到、重放、设备不匹配或已被新操作取代的结果只记日志，不会回滚模式显示。已打开的
> 单台弹窗订阅最新设备快照，模式选中态和待机/退出待机无需等待下一次周期 wall 才刷新。
>
