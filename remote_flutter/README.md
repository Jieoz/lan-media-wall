...
> **当前版本 `1.17.0+67`(`pubspec.yaml`)。CI 从 pubspec 派生 `flutter build apk --build-name=<pubspec name> --build-number=<pubspec code>` 把版本号烧进 APK;播放端 `build.gradle.kts` 也从同一行派生,改 pubspec 即全端同步。发版流程见根 README。本分支 tip 已把 pubspec 临时置为 OTA 诊断构建 `1.17.1-ota-probe+68`(versionCode 68 > 67),用于一次性排查 push 升级卡点(见 player README 的 `UPDATE_STAGE` 说明),非正式发版。
>
