...
def run(repo, cfg):
    p = subprocess.run(["uv", "run", "--with", "pyyaml", "python", str(SCRIPT), str(repo), "--config", str(cfg), "--json"], capture_output=True, text=True)
...
with tempfile.TemporaryDirectory() as td:
    ...
    cfg = repo / "cfg.yml"
    cfg.write_text("command_gates:\n  - {name: policy, run: [python3, ok.py]}\narchive_contracts:\n  - {path: bundle.zip, required_entries: [tool, README.md]}\n")
    ...
    cfg.write_text("command_gates:\n  - {name: policy, run: [python3, bad.py]}\n")
    rc, data = run(repo, cfg)
