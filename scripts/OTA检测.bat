...
echo 正在分析:"%BUNDLE%"
"%EXE%" --profile "%PROFILE%" --human analyze "%BUNDLE%" > "%RESULT%" 2>&1
set "RC=%ERRORLEVEL%"
if not "%RC%"=="0" (
...
rem ============================================================
rem  统一失败处理:交互模式下暂停以便操作员查看错误;
rem  无人值守(LMW_OTA_NONINTERACTIVE)模式返回非零退出码。
rem ============================================================
:fail
if not defined LMW_OTA_NONINTERACTIVE (
  echo.
  pause
)
endlocal
