...
- 旧版未携带 `transport_mode` 的命令按兼容规则解释：非空 endpoint 为 `broker`，空 endpoint
  为 `p2p`；普通“清空并恢复自动发现”必须由新版发送方明确写 `auto`。
...
`rejected` 元素为 `{field,reason}`，常见 reason 为 `conflict`、`unknown_field`、`high_risk_transport`、`high_risk_secret`、`invalid_value`、`unsigned_frame`。Broker 和 P2P 均必须把该帧交给控制端；不允许只日志记录后丢弃。
...
> 不得广播，也不得接受其他 Player 伪造或重放同一请求的终态回执；
