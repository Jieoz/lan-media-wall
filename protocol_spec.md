...
```json
{"config_capabilities":{"safe_fields":["device_name","group_id","volume","muted"],"transport_configure":true,"rotate_device_key":true,"config_version":1},"config_snapshot":{"revision":7,"values":{"device_name":"二号厅右屏","group_id":"hall-2","volume":70,"muted":false,"psk_configured":true},"transport":{"broker_host":"192.168.1.10","broker_port":8770,"use_wss":false,"auto_discovery":false},"pending":{},"requires_restart":false}}
```
...
```json
{"type":"transport_configure","payload":{"device_id":"and-hall-03","request_id":"transport-...","broker_host":"192.168.1.10","broker_port":8770,"use_wss":false,"rollback_timeout_ms":30000}}
```

- 这是独立命令，播放器校验主机和端口后写盘，并**先经旧链路**回报
  `pending.transport:"reconnecting"`，再重建 transport。
- 空 `broker_host` 表示清空 broker、恢复自动发现/P2P；broker 不修改自身监听配置。
...
| `transport_configure` | `device_id`,`request_id`,`broker_host`,`broker_port?`,`use_wss?` | 高风险连接重建，独立确认/回执 |
