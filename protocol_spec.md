...
| `update_status` | player→coordinator | `device_id`,`state`,`detail?`,`version_code` | 回报进度:`downloading`/`installing`/`rejected`/`failed` |

- `url` 指向 broker 媒体库(§20 HTTP 端口)上的 APK,支持 Range 断点续传。
- `version_code` 为**目标 APK 的 Android versionCode**(整数,单调递增)。

### 23.2 四条安全护栏(被控端**必须**全部满足才安装)
1. **鉴权帧**:`update_app` 帧的 §3 签名**必须验签通过**(`authed`)。`auth_mode=open` 或空签名的帧一律拒
   (`rejected:unauthenticated`)。即被控端**只接受已配置密钥并验签通过的自更新指令**,防局域网内任意端刷机。
...
### 23.3 边界
- 仅内网。被控端 4.4 明文存储密钥,`update_app` 依赖 §13 `auth_mode`≠`open` + 已配 PSK 才生效;
  **切勿把被控端暴露公网**(否则等于开放远程 root 装包)。
