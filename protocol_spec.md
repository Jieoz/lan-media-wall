...
{"type":"configure_device","payload":{
  ...
  "volume":70,                 // 可选:持久音量偏好
  "muted":false,               // 可选
  "broker_host":"192.168.1.10",// 可选:写播放端 broker 主机;空串=清空回发现/P2P
  "broker_port":8770,          // 可选:1–65535
  "use_wss":false,             // 可选
  "psk":"..."                  // 可选:仅已鉴权帧可改;open 链路播放端拒改
}}
```
- 只处理 payload 中出现的字段;缺省字段不动。
- broker 处理:更新注册表(`device_name`/`group_id` 持久化)→ 转发给目标 player(`to:"player:<id>"`)
  让其应用本地偏好(如 `volume`/`muted`/连接字段)→ wall dirty。连接字段 broker 只透传,不改 broker 自身监听。
- player 处理连接字段:写盘 → **重建 transport**(停旧 link/discovery 后按新 Settings 再选)→ status 继续上报。空 `broker_host` 清空回自动发现/P2P。
- `psk` 变更要求入站帧 `env.authed=true`;未鉴权拒改密钥,其它字段仍可应用。
...
| `configure_device` | `device_id`,`device_name?`,`group_id?`,`volume?`,`muted?`,`broker_host?`,`broker_port?`,`use_wss?`,`psk?` | 设备配置统一入口 (§19);连接字段写盘并重建 transport;psk 需鉴权 |
