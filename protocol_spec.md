...
| `download_logs_result` | player→(broker/P2P)→controller | `device_id`,`ok`,`filename?`,`content?`,`truncated?`,`error?` | 回传日志文本;单帧上限由端实现控制,超限需标记 `truncated:true` |
| `debug_snapshot` | controller→(broker/P2P)→player | `device_id` 或 envelope `to=player:<id>` | 请求目标被控端生成当前诊断快照 |
| `diagnostic_status` | player→(broker/P2P)→controller | `device_id`,`ok`,`snapshot?`,`error?` | 回传版本、播放态、缓存、最近错误、helper/root/update 探针等诊断信息 |
