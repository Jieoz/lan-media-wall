...
- **规模**:遥控端要同时维持 N 条连接并本地扇出,**适合小规模(经验值 ≤8 台)**;30 屏仍应用模式 A/B。v1.14.11 起 P2P 媒体传输采用两级有界背压:控制端临时 HTTP 服务最多同时流式发送 6 条(等待队列最多 64,再超限返回 `503 Retry-After: 1`),每台 Android 播放端最多同时下载 2 项(队列最多 64,超限在 `status.cache[item_id]` 报 `error:queue-full`)。Range 续传、sha256 校验和本地缓存播放合同不变;该边界防止 `设备数 × 列表项数` 放大为无界线程/连接,但不把 P2P 扩展为大墙分发架构。
