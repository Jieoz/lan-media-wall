...
## [v1.18.6 Clean Install → OTA Qualification] — 2026-07-21

- **Corrected end-to-end target** (versionCode **1186**, versionName `1.18.6`). Provision a clean device with the corrected `1.18.5 (1185)` bundle, then select this APK in the manifest-derived controller flow to prove a USB-free `1185 → 1186` update.
...
- **Operator-UX rebuild** (versionCode **1180**, versionName `1.18.0`). Single-sourced from `remote_flutter/pubspec.yaml` (`1.18.0+1180`). Mapping: `versionCode = major×1000 + minor×10 + patch`. P2P stays the primary path; Broker remains advanced/secondary. No protocol, transport, exact-device `device_id` routing, cache/OTA/safety contract changed — this is a UX-layer rebuild.
  - **Controller — settings topology truth.** New connection-mode control **自动发现 / P2P（推荐）** vs **连接 Broker（高级）**, persisted (`settings.connection_mode`) with migration (a stored broker host migrates to Broker mode; otherwise auto/P2P). P2P mode hides broker host/port/WSS/upload-token; controller identity stays available but collapsed. The connection label is derived from **actual** topology + peers/state (`P2P · 已连接 N 台` / `P2P · 正在发现设备` / `Broker · 已连接` / `Broker · 重连中`) via the single-source `WallState.connectionStatusLabel` — never optimistic. Broker port is strictly validated `1–65535` inline with **no silent `8770` fallback**. Saving says `设置已保存，正在重新连接`; the connected state comes from real state transitions.
  - **Controller — unified single-device push.** Orchestration `推送到此设备` and the device-card `推送内容` now funnel into one shared `showPushToDeviceDialog` (push_workflow.dart). The confirmation documents target, item count, replace/append, cache behavior, and whether playback starts; the two explicit final choices are **仅下发并缓存** and **缓存完成后播放**. Exact-device unicast routing and the upload/cache barrier are preserved.
  - **Controller — destructive + truthful wording.** Local draft clear renamed **清空本地草稿** (states it touches no device). Remote clear renamed **停止播放并清空设备列表** with a red destructive confirmation stating the exact target (`该设备` / `该组` + known-online count), the effect (stop + black screen + list clear), and that cached files are retained; confirm button `停止并清空 …`. Before a device ACK, single-device transport (pause/resume/stop/prev/next), remote clear, and append now say `命令已发送，等待设备确认` instead of claiming `已清空 / 已暂停 / 已停止`.
  - **Android Player — setup rebuild.** `activity_settings.xml` reorganized into a **primary 播放端设置 card** (device name/version/IP/device-id, live connection status, one-tap **刷新连接**, large P2P pairing QR, **保存并启动**) + operator-relevant playback settings + one-time field takeover, then a **collapsed 高级网络与诊断** section (broker/WSS/PSK, hardware/self-check, diagnostics, root daemon, export, reset). The pairing QR appears before any diagnostics. The advanced section uses an API19-safe `GONE/VISIBLE` toggle (no androidx expandable dependency) preserving the 4.4 target and TV D-pad reachability. Primary visible labels are Chinese in both default and `zh-rCN` string files; raw tokens/English stay in the collapsed diagnostics/export.
  - **Tests.** Adds Dart unit/widget tests (`connection_status_test.dart`, `push_workflow_test.dart`, `settings_topology_test.dart`, run in cloud CI) and a locally-runnable Python static-contract suite (`scripts/tests/test_ux_optimization_contracts.py`, 16 contracts) pinning the eight UX contract areas at source level.
