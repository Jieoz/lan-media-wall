...
- The daemon is now built **fully static + non-PIE** (`-static -no-pie -fno-PIE`). A static binary carries its own libc, so there is nothing to resolve against the device bionic — it runs identically on API19..current and the "which API level exports which symbol" question is moot for every syscall wrapper. Non-PIE avoids the static-PIE loader path (unreliable on 4.4 kernels). The NDK's min API is 21, so it compiles with the api21 clang (headers only) but links static.
