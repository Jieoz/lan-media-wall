...
>
> **v1.17.4 (player experience + remote broker configure):**
> Selected cache cleanup no longer StackOverflows on Android (`cacheSummary` →
> outer `buildCacheSummaryMap`). API19 single-decoder media swaps use a near-
> fullscreen freeze JPEG hold frame. Settings binds the physical “回主页” key
> (`KEYCODE_SETTINGS`=176). Controllers can push/clear per-device broker
> transport via `configure_device` (`broker_host`/`broker_port`/`use_wss`/`psk`);
> players persist and rebuild the link without stacking status loops. PSK needs
> authenticated/signed frames.
>
...
**当前版本:`v1.17.5`**(单一真相源 `remote_flutter/pubspec.yaml` = `1.17.5+1175`;player `build.gradle.kts` 与 flutter-build 都从它派生 versionName/versionCode,tag 名也据此,四处不再手动同步、不再漂移)。

> **`1.17.5+1175` 是正式产品修复**(versionCode 1175,严格新于 1174)。在 1.17.4 播放体验与远程连接配置之上,补齐现场两处硬伤:**控制端重命名播放端**后设备墙立刻显示新名(`status.device_name` + Discovery 热更新 + 改名立即 status);**推送升级**在 `and-8b0677b40b` 类盒子上不再因 4.4/YunOS 的 noisy `pm` exit 把真 `Success` 误报为 `pm_failed`,且失败 detail 优先给出 `Failure`/`Error` 而非仅 `pkg:` 暂存路径。控制端更新对话框仍接受直观号/版本名(`v1.17.5`/`1175`/`1.17.5+1175`,规则 major×1000+minor×10+patch)。**运维要点**:OTA 安装逻辑在 root 守护进程内——失败类盒子需用 **QZX Update Tools / `lmw_setup` 重装新 `lmw_root_daemon`**,只推 Player APK 不够。**残余风险**:若 pm 真失败(签名不一致等),仍会诚实 `pm_failed`;legacy 整机重启仍是最后手段。
