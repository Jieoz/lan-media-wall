...
**当前版本:`v1.17.0`**(单一真相源 `remote_flutter/pubspec.yaml` = `1.17.0+67`;player `build.gradle.kts` 与 flutter-build 都从它派生 versionName/versionCode,tag 名也据此,四处不再手动同步、不再漂移)。

> **本分支 tip 是一次性 OTA 诊断构建:`1.17.1-ota-probe+68`**(versionCode 68,严格新于现场基线 67)。`update_app` 路径现向 `player.log` 与 `update_status.detail` 双写机器可读的 `UPDATE_STAGE=…` 标记(daemon_probe / download / sha256 / staged / pm_install / restart_app / legacy_stage / exception),并在下载前对 root 守护做 fail-closed 探测(桥断则停在 `daemon_probe`,不浪费整包下载)。非正式产品发版,上面的 v1.17.0 叙述仍然成立。
