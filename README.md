...
> **v1.18.3 OTA Test:** Player startup now reconciles the APK-embedded root daemon,
> and the controller derives `versionCode`/`versionName` from the selected APK
> manifest instead of accepting a hand-entered target. This is a test candidate,
...
**当前测试版本:`v1.18.3`**(单一真相源 `remote_flutter/pubspec.yaml` = `1.18.3+1183`;Player 启动会校验/更新内嵌 daemon；控制端从 APK manifest 读取真实版本号。该版本仅用于 `1182 → 1183` 单机 OTA 闭环验证，不是正式 Release)。
