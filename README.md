...
**当前版本:`v1.17.3`**(单一真相源 `remote_flutter/pubspec.yaml` = `1.17.3+1173`;player `build.gradle.kts` 与 flutter-build 都从它派生 versionName/versionCode,tag 名也据此,四处不再手动同步、不再漂移)。

> **`1.17.3+1173` 是正式产品修复**(versionCode 1173,严格新于 field-fix 71 / boot-probe 70 / formal 69)。在 69 OTA 修复与 70/71 现场取证/分组回读之上,补齐设备墙筛选、缓存多选、单台列表入口与无 ADB 接管快捷入口。基于现场证据修 root-box OTA:root 守护**先做 App 级安装再考虑整机重启**——`pm install -r` → `pm install -r -f`(强制内置),打出 `Success` 即只重启 App、不整机重启;仅当两次 pm 都失败才回退 legacy 暂存激活,发规范行 `ok install state=legacy_activation_dispatched reboot_required`,被控端把它(及现场变体 `legacy_staged`+`reboot_pending`)当作**非失败**的待重启态,不再误报 `install_daemon_fail`。控制端更新对话框接受直观号/版本名(`v1.17.3`/`1173`/`1.17.3+1173`,规则 major×1000+minor×10+patch),拒≤当前值,预填 current+1。`update_app` 仍双写 `UPDATE_STAGE=…` 标记并在下载前对 root 守护 fail-closed 探测。**残余风险**:若某盒子两次 pm 安装都失败,legacy 整机重启仍是最后手段(激活时该设备网络可能瞬断)——本版把这条路径变得诚实且罕见,而非不可能。
