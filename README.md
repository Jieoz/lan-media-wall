...
| 远程日志下载 + 调试快照(v1.13.4) | 单设备详情弹窗在保留「推送升级」的同时新增两个只针对**这一台**的按钮:①**下载日志**——下发 `download_logs`,被控端 `PlayerService.hDownloadLogs` 把持久化的 `player.log`(含滚动尾段,单帧上限 128 KB)经 `download_logs_result` 回传,控制端写入本地临时文件并提示保存路径;②**调试快照**——下发 `debug_snapshot`,被控端 `buildDebugSnapshot` 聚合版本/播放态/缓存/最近错误/helper 探针经 `diagnostic_status` 回传。被控端 `logEvent` 带 256 KB 滚动的落盘日志环;设置页同屏渲染本地诊断面板(helper/restart/update/playback/cache/errors/probe),无需导出。控制端请求侧用按 `deviceId` 归键的 pending completer,回调命中即完成、超时即释放,不再永久挂起 |
