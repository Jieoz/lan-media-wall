...
**当前版本:`v1.17.6`**(单一真相源 `remote_flutter/pubspec.yaml` = `1.17.6+1176`;player `build.gradle.kts` 与 flutter-build 都从它派生 versionName/versionCode,tag 名也据此,四处不再手动同步、不再漂移)。

> **`1.17.6+1176` 是正式产品修复**(versionCode 1176,严格新于 1175)。现场 `and-b2b90f28f7` 在 1174 已走 legacy 成功后,再推 1175 报 `legacy_activation_failed`:根因是成功 stage 留下的 `/data/app/...-1.apk.lmw-backup` 被永久 fail-closed。daemon 现对「target+backup 都在」自动 commit,对「仅 backup」先 restore,再 stage。1.17.5 的改名回显与 `pm Success` 诚实判定保留。**运维要点**:OTA 安装逻辑在 root 守护进程内——**已走过 legacy 的盒子必须用 QZX Update Tools / `lmw_setup` 重装新 `lmw_root_daemon`**,只推 Player APK 不够。临时绕过:root 删 `.lmw-backup` 后重试。
