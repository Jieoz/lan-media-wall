...
> **v1.18.4 OTA Field Test:** Player startup reconciles the APK-embedded root daemon,
> and the controller derives `versionCode`/`versionName` from the selected APK
> manifest instead of accepting a hand-entered target. The staged daemon candidate
> is made executable before its isolated probe. This is a test candidate,
...
**当前测试版本:`v1.18.4`**(单一真相源 `remote_flutter/pubspec.yaml` = `1.18.4+1184`;Player 启动会校验/更新内嵌 daemon；候选 daemon 在隔离探测前获得执行权限；控制端从 APK manifest 读取真实版本号。该版本仅用于 `1182 → 1184` 单机 OTA 闭环验证，不是正式 Release)。
