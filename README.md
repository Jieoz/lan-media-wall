...
> 单台弹窗订阅设备确认结果并刷新不可变状态快照，模式选中态与待机按钮不会停在旧值。
> Broker OTA 现场验收使用 `1.19.2 → 1.19.3`：1.19.1 不可能执行尚未安装的
> `brokerLocal` 授权代码，不通过放宽旧 Guard 绕过。单一版本源为 `1.19.3+1193`。
>
...
> revision 和同模式状态回读后切换自身链路。旧 Controller 未发送 `transport_mode` 时，清空
> endpoint 仍保持原有 `auto` 语义；只有显式 `p2p` 才强制直连。配置结果按请求原控制端
> 单播；兼容无 `request_id` 的旧请求时按目标 Player 串行关联，不向其他 Controller 广播。
> 单一版本源为 `1.19.1+1191`。
>
...
> from the selected APK manifest instead of accepting a hand-entered target. The
> daemon handoff is SHA-256 checked, root/protocol probed, atomically replaced, and
> rollback-protected; an already-current daemon is handled as a successful idempotent
> terminal state. The corrected `1.18.5 (1185) → 1.18.6 (1186)` path completed on a
> real API 19/YunOS box through P2P download, exact hash verification, legacy
> activation, reboot/reconnect, startup reconcile, cache restore, and playback with
> `errors=none`.
>
