...
**当前版本:`v1.11.0`**(player `versionCode 28 / versionName 1.11.0`,controller `1.11.0+28`)。每次发版四处版本号必须同时抬,否则 tag 名与产物内部版本号漂移(装到盒子上 `dumpsys package` 查出来会对不上 tag)。

1. **抬四处版本号**(必须一致):
   - `android_apps/player/app/build.gradle.kts` — `versionCode`(**每次 +1**,安卓靠它判新旧)+ `versionName`
   - `remote_flutter/pubspec.yaml` — `version: X.Y.Z+N`
   - `.github/workflows/flutter-build.yml` — 两处 `--build-name=X.Y.Z --build-number=N`(pubspec 版本不一定进 flutter APK,靠这兜底)
