...
**当前版本:`v1.17.4`**(单一真相源 `remote_flutter/pubspec.yaml` = `1.17.4+1174`;player `build.gradle.kts` 与 flutter-build 都从它派生 versionName/versionCode,tag 名也据此,四处不再手动同步、不再漂移)。

> **`1.17.4+1174` 是正式产品修复**(versionCode 1174,严格新于 1173 / field-fix 71 / boot-probe 70 / formal 69)。在 1.17.3 操作员 UX 与直观升级号之上,补齐播放体验与远程连接配置:勾选清理缓存不再因 `cacheSummary` 自递归崩进程;API19 单解码器切媒体用近全屏冻结帧盖空窗;设置页物理「回主页」键回墙;控制端可对单台推送/清空 broker 连接(`broker_host`/`broker_port`/`use_wss`/`psk`),播放端写盘后只重建链路不叠 status 循环,PSK 需鉴权/签名帧。控制端更新对话框仍接受直观号/版本名(`v1.17.4`/`1174`/`1.17.4+1174`,规则 major×1000+minor×10+patch)。**残余风险**:API19 冻结帧不是双 VDEC 零缝;若某盒子两次 pm 安装都失败,legacy 整机重启仍是最后手段。
