...
> **v1.18.5 Clean Install → OTA Qualification:** Player startup reconciles the APK-embedded root daemon,
...
**当前测试版本:`v1.18.5`**(单一真相源 `remote_flutter/pubspec.yaml` = `1.18.5+1185`;以同一发布中的 `1.18.4 (1184)` QZX 完整安装包为基线，再通过控制端从 APK manifest 读取真实版本并远程升级到 `1185`，用于从安装到升级的单机闭环验收)。
