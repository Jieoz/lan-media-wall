...
| 远程日志下载 + 调试快照(v1.13.4, 协议 §24) | 单设备详情弹窗在保留「推送升级」的同时新增两个只针对**这一台**的按钮:①**下载日志**——下发 `download_logs`,被控端 `PlayerService.hDownloadLogs` 把持久化的 `player.log`(含滚动尾段,单帧上限 128 KB)经 `download_logs_result` 回传,控制端写入本地临时文件并提示保存路径;②**调试快照**——下发 `debug_snapshot`,被控端 `buildDebugSnapshot` 聚合版本/播放态/缓存/最近错误/helper 探针经 `diagnostic_status` 回传。被控端 `logEvent` 带 256 KB 滚动的落盘日志环;设置页同屏渲染本地诊断面板(helper/restart/update/playback/cache/errors/probe),无需导出。控制端请求侧用按 `deviceId` 归键的 pending completer,回调命中即完成、超时即释放;broker 和 P2P 协调端必须显式转发请求/回包,不再永久挂起 |
...
**单台一键更新(Windows,无需 bash)**:给已部署的盒子升级版本时,用 `scripts/lmw_update.bat`。它把新 APK + `scripts/lmw_provision.sh` 推进盒子,跑装包相位触发重启,用 `adb wait-for-device` + 轮询 `sys.boot_completed` **自动跨过那次重启**,再跑校验/自启/清理相位直到 `PROVISION COMPLETE`——从 PC 看就是**一条命令**。
- 装 APK 必须重启让系统收编,而重启会杀掉盒子内正在跑脚本的 shell,所以**盒子端**脚本天然分两相位(装+重启 / 校验+自启+清理),靠 `/data/local/tmp/lmw_phase` 接力;`lmw_update.bat` 只是在 PC 端把两相位自动焊起来。
- 用法(APK 叫什么名都行):把 APK **拖到 `lmw_update.bat` 上**,或 `lmw_update.bat "路径\任意名.apk"`,或文件夹里只放一个 apk 直接双击(多个则取最新)。签名换了要清装则末尾加 `FORCE`。版本号以 APK 内部为准,不用手填。
- 底层的盒子端脚本 `scripts/lmw_provision.sh`(ON-BOX 相位状态机)与还原脚本 `scripts/lmw_restore.sh`(`pm enable` 重启旧桌面/挖矿/bloat + 卸载 player + 清数据 + 重启)也一并归档,可单独 `adb push` 使用,支持"还原→部署→验证"从头验证流程。
...
docs/                # 文档与效果截图
scripts/             # 运维脚本:批量装机 deploy_player.sh、单台一键更新 lmw_update.bat(+ON-BOX lmw_provision.sh / 还原 lmw_restore.sh)、README 同步门槛 check_readme_sync.sh 等
