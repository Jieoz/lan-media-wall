...
> **v1.14.13:** the Android player is now diagnosable at 启动中 without adb — startup is classified (`WAITING_SETUP` / `STARTING` / `START_FAILED`) by a pure `StartupStatusPolicy`, a stalled foreground-service start flips to an on-screen cause within 8s, and an "export diagnostics to a file" button writes the startup phase + settings + `player.log` tail to a USB/file-manager-readable path even when no service is running. The controller's multi-file playlist moves out of transient Widget state into a tested `PlaylistDraft` (multi-select import with `item_id` de-dup + order, reorder/remove/clear, load-from-active-playlist), release lanes/tag promotion resolve one checked-in public production-certificate fingerprint (with an optional repository-variable override) and fail closed on signer + pubspec `versionName/versionCode`; controller startup now cancels an in-flight async `WallState.init()` cleanly when its provider is disposed, preventing post-dispose links/discovery/notifications.
...
**当前版本:`v1.14.13`**(单一真相源 `remote_flutter/pubspec.yaml` = `1.14.13+61`;player `build.gradle.kts` 与 flutter-build 都从它派生 versionName/versionCode,tag 名也据此,四处不再手动同步、不再漂移)。
