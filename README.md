...
**当前版本:`v1.18.1`**(单一真相源 `remote_flutter/pubspec.yaml` = `1.18.1+1181`;player `build.gradle.kts` 与 flutter-build 都从它派生 versionName/versionCode,tag 名也据此,四处不再手动同步、不再漂移)。

> **`1.18.1+1181` 发布 Windows 无 Python 现场 OTA 检测器。**Release 的 QZX Update Tools 资产包含 UTF-8 BOM/CRLF 双击启动器 `OTA检测.bat` 与独立 Windows x64 `android_ota/android_ota_diag.exe`;无需安装 Python即可离线生成中文证据报告。该版本只改工具/打包层,不改协议、传输、路由或设备端 OTA 安装合同。
