...
**当前版本:`v1.17.7`**(单一真相源 `remote_flutter/pubspec.yaml` = `1.17.7+1177`;player `build.gradle.kts` 与 flutter-build 都从它派生 versionName/versionCode,tag 名也据此,四处不再手动同步、不再漂移)。

> **`1.17.7+1177` 正式发布通用 Android OTA 诊断框架**(versionCode 1177)。Release 资产 `LANMediaWall-v1.17.7-QZX-Update-Tools.zip` 内含 `android_ota/android_ota_diag.py` 与 profile(`standard-pm` / `qzx-yunos-4.4`);主机仿真器编译生产侧 `lmw_legacy_stage` 覆盖二次 OTA leftover-backup / orphan-backup。1.17.5/1.17.6 的安装诚实与 leftover commit 行为保留。**运维要点**:现场装机逻辑仍在 root 守护进程内——缺新 daemon 的盒子仍需 QZX Update Tools / `lmw_setup`;诊断器只做离线判定,不替代装机。
