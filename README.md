...
> **v1.14.13:** the Android player is now diagnosable at 启动中 without adb — startup is classified (`WAITING_SETUP` / `STARTING` / `START_FAILED`) by a pure `StartupStatusPolicy`, a stalled foreground-service start flips to an on-screen cause within 8s, and an "export diagnostics to a file" button writes the startup phase + settings + `player.log` tail to a USB/file-manager-readable path even when no service is running. The controller's multi-file playlist moves out of transient Widget state into a tested `PlaylistDraft` (multi-select import with `item_id` de-dup + order, reorder/remove/clear, load-from-active-playlist), and release lanes/tag promotion fail closed on the production signer + pubspec `versionName/versionCode`.
